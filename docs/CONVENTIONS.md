...
- Jobs:
  - `lint-api` — runs `ruff check` (pinned via `uvx ruff@<version>`). Config
    lives in `api/pyproject.toml` (`[tool.ruff]`); the rule set is the
    high-signal `["E4", "E7", "E9", "F", "I"]` (pyflakes + pycodestyle errors +
    import sorting), and autogenerated `niouzou/migrations/versions` is excluded.
    ruff is deliberately **not** a locked dependency so the torch-CPU-index lock
    stays resolvable offline — it runs standalone.
  - `test-api` — boots a PostgreSQL 17 service container, applies Alembic
    migrations, then runs the pytest suite via `uv`.
  - `test-pwa` — installs PWA deps with `npm ci`, runs `npm run lint` (eslint),
    then `npm run build` (TypeScript project references + Vite production build).
- Run the linters locally before pushing: `uvx ruff check api` and
  `cd pwa && npm run lint`.
