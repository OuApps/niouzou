...
Optional: `OPENROUTER_API_KEY`, `OPENROUTER_MODEL`, `CHAT_MODEL` (article chat, falls back to `OPENROUTER_MODEL` — E21), `CHAT_WEB_SEARCH` (internet search in the chat — E21-S7), `FRONTEND_URL` (front-end base URL for user-facing links: shareable `/article/{id}` links the MCP returns — E23-S2; legacy `PUBLIC_APP_URL` still read as fallback), `SCORE_THRESHOLD`, `RANDOM_SURFACE_RATE`, `FEED_GRAVITY`, `SCORING_MODE` (`keyword`|`smart`), `CRON_NIGHTLY_REFRESH_HOUR` (legacy `CRON_REFRESH_WEIGHTS_HOUR` still read as fallback), `SMART_*` (E16 knobs)
